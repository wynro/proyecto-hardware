...
void IISClose() {
	...
	/* recover Port B and Port C state */rPDATB = 0x7ff;	// P9-LED1 P10-LED2
	rPCONB = 0x1cf;
	rPDATC = 0xff00;
	rPCONC = 0x0ff0ffff;
	rPUPC = 0x30ff;
...
void Playwave(int times) {
	...
	/* enable interrupt */rINTMOD = 0x0;
	rINTCON = 0x1;
	/* execute command "download t.wav 0x0c300000" before running */
	pWavFile = (unsigned short*) 0x0C310000;

	/* initialize philips UDA1341 chip */
	Init1341(PLAY);
	/* set BDMA interrupt */pISR_BDMA0 = (unsigned) BDMA0_Done;
	...
	for (i = 0; i < times; i++) {
		...
		iDMADone = 0;
		sound_len = 1048570;		//475136;//512000;// 155956;

		rBDISRC0 = (1 << 30) + (1 << 28) + ((int) (pWavFile));// Half word,increment,Buf
		rBDIDES0 = (1 << 30) + (3 << 28) + ((int) rIISFIF );// M2IO,Internel peripheral,IISFIF
		/* IIS,Int,auto-reload,enable DMA */rBDICNT0 = (1 << 30) + (1 << 26)
				+ (3 << 22) + (0 << 21) + (1 << 20) + sound_len;
		rBDCON0 = 0x0 << 2;

		/* IIS Initialize */rIISCON = 0x22;	// Tx DMA enable,Rx idle,prescaler enable
...
void Record_Iis(void) {
	...
	Init1341(RECORD);
	/****** BDMA0 Initialize ******/rBDISRC0 = (1 << 30) + (3 << 28)
			+ ((int) rIISFIF );	// Half word,inc,Buf
	rBDIDES0 = (2 << 30) + (1 << 28) + ((int) rec_buf);	// M2IO,fix,IISFIF
	rBDICNT0 = (1 << 30) + (1 << 26) + (3 << 22) + (1 << 21) + (1 << 20)
			+ REC_LEN;
	rBDCON0 = 0x0 << 2;

	/****** IIS Initialize ******/rIISCON = 0x1a;// Rx DMA enable,Rx idle,prescaler enable
	...
	size = *(Buf + 0x2c) | *(Buf + 0x2d) << 8 | *(Buf + 0x2e) << 16
			| *(Buf + 0x2f) << 24;
	size = (size >> 1) << 1;
	//size=REC_LEN*2;
	//Uart_Printf(" size=%d\n",size);
	...
	/****** IIS Initialize ******/rIISCON = 0x26;// Tx DMA enable,Rx idle,prescaler enable
