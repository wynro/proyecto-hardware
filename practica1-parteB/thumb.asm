...
sudoku_candidatos_thumb:
	...
	lsr r4, #12
	mov r5, r4 // Lo movemos cortando el numero
	lsl r4, #12
	...
	// Ponemos todas las pistas a 1, metiendo la constante 0x1FF
	mov r5, #16
	lsl r5, #4
	orr r4, r5
	mov r5, #240
...
sudoku_candidatos_thumb_bucle_fila_columna_si_fila_cero:
	...
	mov r7, #1 // Ponemos el 1 de la futura mascara
	lsl r7, r6
	mvn r7, r7 // Movemos el 1 a su posicion y lo negamos para convertirlo en una mascara
	and r4, r4, r7 // Aplicamos la mascara a nuestra celda
...
sudoku_candidatos_thumb_bucle_fila_columna_si_columna_cero:
	...
	bpl sudoku_candidatos_thumb_no_region_fila_3
	mov r5, #3 // Es negativa, estamos en las region 0 o 1, asumimos 1
sudoku_candidatos_thumb_no_region_fila_3:
	cmp r1, #3 // Si la resta fila - 3 ...
	bpl sudoku_candidatos_thumb_no_region_fila_2
	mov r5, #0 // Es negativa, estamos en la region 0
sudoku_candidatos_thumb_no_region_fila_2:
	...
	mov r5, #6
	cmp r2, #6
	bpl sudoku_candidatos_thumb_no_region_columna_3
	mov r5, #3
sudoku_candidatos_thumb_no_region_columna_3:
	cmp r2, #3
	bpl sudoku_candidatos_thumb_no_region_columna_2
	mov r5, #0
sudoku_candidatos_thumb_no_region_columna_2:
	mov r2, r5


	#
	# r0: contador de fila
	# r3: contador de columna
	#
...
sudoku_candidatos_thumb_tag_columna:
	...
	mov r5, r3 // Movemos a r5 el valor de la direccion inicial de la cuadricula
	add r7, r1, r0 // Almacenamos en r7 la suma del contador de fila y la fila inicial de la region
	lsl r7, #5
	add r5, r5, r7 // Le sumamos a la dirección inicial el offset de fila
	lsr r7, #5
	add r7, r2, r3 // Almacenamos en r7 la suma del contador de la columna y la columna inicial de la region
	lsl r7, #1
