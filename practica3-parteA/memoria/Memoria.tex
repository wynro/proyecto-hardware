...
%% \textcolor{red}{ Describe los pasos realizados para llegar hasta
%%   los resultados. Todas aquellas decisiones de diseño tomadas en el
%%   proceso deben incluirse.En la asignatura de Proyecto Hardware se
%%   debe explicar en este apartado el esquema del proyecto (ficheros
%%   y funciones que lo componen), número total de líneas de código y
...
fichero fuente, a excepción del módulo \texttt{sudoku\_2015}, que
también incluye el fichero sudoku\_candidatos.asm, el cual implementa
algunas funciones de alta carga de procesado en ensamblador.
  ...
  \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=5cm,semithick]
    ...
    \path (A) edge [bend left    ] node {\leftstack{next \&\\v=4}}               (B)
          ...
          (H) edge [bend right=90] node {\leftstack{next \&\\errores!=0}}        (E)
              ...
              ;
  \end{tikzpicture}
  \caption{Diagrama de estado del juego de sudoku\\Dada su
    complejidad, se ha decidido no expandir las acciones realizadas en
    cada estado}
...
\item Dentro de la parte de tratamiento de rebotes tenemos:
  ...
  \item \textbf{Ser capaces de depurar un código con varias fuentes de
    ...
    (\ref{subsubsec:pila-debug}{sección}) no se tuvieron problemas al
  ...
  \item \textbf{Gestionar la entrada/salida con dispositivos básicos,
    asignando valores a los registros internos de la placa desde
    C:}\\ Gracias a las librerías de desarrollo de la placa, fue
    sencillo el uso de los periféricos.
  \item \textbf{Aprender a desarrollar en C las rutinas de tratamiento
    ...
    atributos del compilador usado (\texttt{\_\_attri-}
    \texttt{bute\_\_((interrupt(``IRQ'')))}) y el evitar las rutinas
    de interrupción demasiado grandes, no se tuvo ningún problema
    durante la consecución de este objetivo.
  \end{itemize}
\item Dentro de la segunda parte:
  \begin{itemize}
