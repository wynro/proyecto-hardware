...
void Lcd_Draw_HLine_pointed(INT16 usX0, INT16 usX1, INT16 usY0, INT8U ucColor,
		INT16U usWidth) {
	INT16 usLen;
...
void Lcd_Draw_VLine_pointed(INT16 usY0, INT16 usY1, INT16 usX0, INT8U ucColor,
		INT16U usWidth) {
	INT16 usLen;
...
// TODO: PUES ME HARÉ MI PROPIO LCD; CON CASINOS, Y FURCIAS!
	...
	for (y = 0; y < 16; y++) {
		for (x = 0; x < 8; x++) {
			k = x % 8;
			if (g_auc_Ascii8x16[ulOffset + y] & (0x80 >> k)) {
				xx = usX0 + x;
				LCD_PutPixel(xx, y + usY0, (INT8U)ForeColor);
			}
		}
	}
}
...
void ReverseSquare(INT32U ulX0, INT32U ulY0, INT32U ulX1, INT32U ulY1) {
	...
	for (x = ulX0; x < ulX1 + 1; ++x) {
		for (y = ulY0; y < ulY1 + 1; ++y) {
			LCD_PutPixel(x, y, (INT8U) (BLACK - LCD_GetPixel(x,y)));
		}
	}
}
