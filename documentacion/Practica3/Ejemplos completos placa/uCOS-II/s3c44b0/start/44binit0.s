...
.equ 	MODEMASK,	0x1f
.equ 	NOINT,		0xc0
.equ    CPSR_IRQ_EN,	0x80       /* disable Interrupt Mode (IRQ) */
...
.endif
    ...
    #*	Initialize stacks								* 
    #****************************************************
    ldr	    sp, =SVCStack		/* CPU works in SVC-mode after reset.	*/		
    bl	    InitStacks
    ...
    #********************************************************
    #*	Copy and paste data and zero initialized data	    *
    #********************************************************
    LDR	    r3, =Image_ZI_Base	/* Zero init base => top of initialised data */

    /* RUN in FLASH or Debugging in RAM:
       Please DO NOT define RAM_RUN when program onto flash.
       Then use the linker script file flash.ld to generate the BIN file. */
   .IFNDEF RAM_RUN
...
F0:
    ...
    /* RUN in RAM or Debugging in RAM:
       Please define RAM_RUN when program onto flash.
       Then do not need to chage the linker script file any more. */
   .IFDEF RAM_RUN
   .EQU ROM_BASE, 0x0
    LDR	    r0, =ROM_BASE	    /* Get pointer to RO data                    */
    LDR	    r1, =Image_RW_Base	/* copy	end point                             */
    LDR		r2, =Image_RO_Base  /* RO code space, defined in the script file */
    SUB		r1, r1, r2          /* Size of code                               */
F00:
    CMP	    r0, r1				/* Copy RO data to RAM space            */
    LDRCC   r4, [r0], #4        /* --> LDRCC r4, [r0] + ADD r0, r0, #4	 */
    STRCC   r4, [r2], #4        /* --> STRCC r4, [r1] + ADD r2, r2, #4   */ 
    BCC	    F00
   .ENDIF
...
InitStacks:
    ...
    orr	    r1,r0,#IRQMODE|CPSR_FIQ_MASK
    msr	    cpsr_cxsf,r1 	    /* IRQMode */
    ldr	    sp,=IRQStack
	
    orr	    r1,r0,#FIQMODE|CPSR_IRQ_MASK
...
.ifeq BUSWIDTH-16
	.long 0x11110102		/* Bank0=16bit BootRom(SST39VF160) :0x0 */
.else
